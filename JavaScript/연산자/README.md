...
삼항 조건 연산자는 조건식의 평가 결과에 따라 반환할 값을 결정한다. **자바스크립트의 유일한 삼항 연산자이며, 부수효과는 없다**. 다음과 같이 사용한다.
...
2. 첫 번째 피연산자가 false로 평가되면 세 번째 피연산자를 반환

간단히 말하면 위와 같고 길게 설명하면 다음과 같다.

물음표 앞의 첫 번째 피연산자는 조건식, 즉 불리언 타입의 값으로 평가될 표현식이다. 만약 조건식의 평가 결과가 불리언 값이 아니라면 불리언 값으로 암묵적으로 타입 변환한다. 이 때 조건식이 참이면 콜론 앞의 두 번째 피연산자가 평가되어 반환되고, 거짓이면 콜론 뒤의 세 번째 피연산자가 평가되어 반환된다.
...
위의 식을 if else문으로 표현해도 삼항 조건 연산자 표현식과 유사하게 처리할 수 있다.
...
하지만 삼항 조건 연산자와 if else문은 중요한 차이가 있다. **삼항 조건 연산자는 값처럼 사용할 수 있지만 if else문은 값처럼 사용할 수 없다**는 것이다.

```jsx
var x = 10;

// if elsea문은 표현식이 아닌 문이다. 따라서 값처럼 사용할 수 없다.
var result = if(x % 2) {
								result = '홀수';
						 } else {
							 result = '짝수';
...
**삼항 조건 연산자 표현식은 값으로 평가할 수 있는 표현식인 문이다.** 따라서 조건에 따라 어떤 값을 결정해야 한다면 if else문보다 삼항 조건 연산자 표현식을 사용하는 편이 유리하다. 하지만 조건이 여러개라면 if else문의 가독성이 더 좋다.
